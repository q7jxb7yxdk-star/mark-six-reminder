...
App 提供 $8,000,000、$13,000,000、$18,000,000 及 $24,000,000 四個固定選項，新安裝預設使用 $13,000,000。設定頁不提供自訂輸入或獨立的「目前門檻」列；勾號直接標示目前選項。已安裝用戶如已有舊自訂門檻，更新後會保留原值，直至用戶選擇新的固定門檻。

「刪除舊號碼」預設關閉。開啟後，首頁只有在成功取得目前攪珠資料，並到達該期香港攪珠日 00:00 後才呼叫 `SavedNumberCleanup`；如 App 正在前景，會等待至該日開始，如由背景返回亦會重新檢查。helper 解析目前及已儲存記錄的 ISO 8601 攪珠日期，只刪除日期較早的 `SavedNumberEntry`，保留目前期數的全部組別。無目前資料、日期無法解析或尚未到攪珠日時不會刪除；SwiftData 儲存失敗時會 rollback 並在首頁顯示錯誤。此功能使用現有 `drawNumber` 及 `drawDate` 欄位，不需要 SwiftData schema migration。
...
頁面依次顯示玩法選擇器、膽／拖選擇器（只限膽拖）、1 至 49 號碼區、操作提示與選號摘要合併卡片，以及操作按鈕。合併卡片放在號碼區下方，集中顯示目前玩法規則、已選數量及組合數。
...
Cloudflare Cron 使用 UTC。以上分別等同香港時間逢星期日、星期二、星期四及星期六 09:15、21:40 及 22:00。09:15 排程更新資料及判斷 APNs 通知；兩個晚間排程更新攪珠結果與最新估計頭獎基金，其中 22:00 亦處理官方資料延遲。晚間排程只更新 D1/KV，不會發送頭獎基金通知；App 仍會於 21:40 及 21:50 強制讀取 Worker 當時已有的資料，用戶亦可下拉重新整理。
...
Production 啟用 09:15、21:40 及 22:00（香港時間）的自動更新排程。部署後檢查：
