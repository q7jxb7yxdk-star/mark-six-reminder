...
| Key | 用途 | 預設值 |
|---|---|---:|
| `notification.threshold` | 通知金額門檻 | 20,000,000 |
| `notification.enabled` | 是否啟用通知 | `true` |
| `notification.installationId` | 每次 App 安裝的穩定 UUID | 首次啟動產生 |

預設門檻為 HK$16,000,000、HK$20,000,000、HK$30,000,000，亦接受 0 至 1,000,000,000 的整數自訂值。
...
Cloudflare Cron 使用 UTC，因此以上等同香港時間每日 09:15。每日執行而不是硬編碼星期，是為了讓官方資料決定當日是否有攪珠，亦可涵蓋星期安排或特別場次變更。
