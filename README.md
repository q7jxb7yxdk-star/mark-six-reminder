...
- 首頁按攪珠期數把多組已儲存號碼分組，每組標題包含期數及組別，官方結果只顯示一次，並逐一標示玩法、命中號碼及頭獎至七獎資格；記錄可向左滑動刪除
- 設定頁可選擇自動刪除早於目前攪珠期數的已儲存號碼；預設關閉，關閉時保留所有期數
...
- 結果 Cron：`45 13 * * SUN,TUE,THU,SAT`（UTC，即香港時間 21:45）
- 後備結果 Cron：`15 14 * * SUN,TUE,THU,SAT`（UTC，即香港時間 22:15）
